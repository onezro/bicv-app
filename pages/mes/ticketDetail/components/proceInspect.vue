	...
	<view class="detail">
		<view class="content">
			<uni-section :title="option.title" titleFontSize="20px" type="line">
				...
				<view class="check-content" v-for="(item,i) in list" :key="item.Step">
		...
		<view class="bottom">
			...
			<view class="choose">
				<view class="decision" v-if="!judgmentFun">
					...
					<button class="custom-style" style="color: #ffffff;backgroundColor:#bdbdbd;borderColor:#bdbdbd"
						@click="taskPassed(3)">
						故障
					</button> -->
					...
					<button class="custom-style" type="primary" @click="taskPassed(9)">
						OK
					</button>
				</view>
			</view>
		...
		<view>
			...
			<uni-popup ref="alertDialog" type="dialog">
				<uni-popup-dialog type="error" cancelText="否" confirmText="是" title="确认提示" content="是否提交结果"
...
<script setup>
	...
	onMounted(() => {
		submitData.inspectOrder = option.order
		submitData.step = option.step
		submitData.subStepList = list.map(item => {
			return {
	...
	const change = (a) => {
		submitData.subStepList[parseInt(a.target.id)].submItemRemark = a.target.value
	}

	const changeValue = (e) => {
		submitData.subStepList[parseInt(e.target.id)].subItemSolution = e.target.value
	}
	...
	const ngRemark = () => {
		...
		let isEmpty = submitData.subStepList.findIndex(item => item.subItemSolution == '')
		if (isEmpty == -1) {
	...
	const dialogInputConfirm = (value) => {
		if (value) {
			submitData.subStatus = 1
			imgDataProce(1, value)
			// submitProcess(submitData).then(res => {
			// 	judgment.value = true
			// 	goToNextStep()
			// 	console.log(res);
			// }).catch(er => {
	...
	const dialogConfirm = () => {
		// submitProcess(this.submitData).then(res => {
		// 	this.judgment = true
		// 	this.goToNextStep()
		// 	console.log(res);
		// }).catch(er => {
