			...
			<view class="box">
				<up-form labelPosition="left" :model="formData" ref="formRef" labelWidth="65">
					...
					<up-form-item label="计划单号" prop="orderNum" class='test'>
						<up-input style="height: 30px;" fontSize='13px' :disabled="true" v-model="formData.OrderNumber">
						</up-input>
					</up-form-item>
		...
		<uni-section title="入库列表" type="line" class="sect" style="flex: 1;">
				...
				<uni-list>
					<uni-swipe-action>
						<uni-swipe-action-item v-for="f in  list" :key="f" :right-options="rightOptions"
							...
							<uni-list-item :title="'烧录条码:'+f.ContainerName" :note="'工单:'+f.ERPOrder+'\n产品编码:'+f.ProductName"
						:rightText="'数量:'+f.Qty"/>
			...
			<template v-slot:right>
				<view class="" style="display: flex;align-items: center;">
					<up-button class="custom-style" text="重置" style="margin-right: 10px;" @click="restForm"></up-button>
			   ...
			   <!-- <up-button class="custom-style" style="margin-left: 10px;"   type="primary" :disabled="list.length==0" text="提交入库"
				...
				<up-button class="custom-style" type="primary" style="margin-left: 10px;" :disabled="list.length==0" text="提交入库"
					@click="onSubmit"></up-button>
				</view>
			  </template>
