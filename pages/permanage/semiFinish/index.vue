		...
		<uni-section title="入库" type="line" class="sect1">
			<view class="box">
				<up-form labelPosition="left" :model="formData" ref="formRef" labelWidth="65">
					...
					<up-form-item label="计划号" prop="orderName" class='test'>
						<up-input placeholder="请输入计划单号" style="height: 30px;" fontSize='13px' :focus="true"
							v-model="formData.orderName"  clearable @confirm="checkOrder">
							<template #suffix>
					...
					<up-form-item label="库位" prop="Loc" class='test'>
						<up-input placeholder="请扫描库位" style="height: 30px;" clearable fontSize='13px'
							:disabled="isOrder" @focus="focus" v-model="formData.Loc" :focus='locFocus' @confirm="getFocus">
							<template #suffix>
								<uni-icons type="scan" color="#bdbdbd" @click="iconGoodQRCode" size="20"></uni-icons>
							</template>
						</up-input>
					</up-form-item>
					<up-form-item label="周转箱条码" prop="barCode" class='test'>
				...
				<uni-list>
					<uni-swipe-action>
						<uni-swipe-action-item v-for="f in  list" :key="f" :right-options="rightOptions"
							...
							<uni-list-item :title="'周转箱条码：'+f.CarrierName" :note="'产品编码：'+f.ProductName"  :rightText="'数量：'+f.TotalQty" />
...
<script setup>
	...
	//监听工单号改变
	watch(() => formData.value.orderName, (newVal, oldVal) => {
		if (newVal !== oldVal) {
			isOrder.value = true
		}
	}, {
...
const getLocFocus = () => {
	...
	//验证工单
	const checkOrder = () => {
		CheckOrder(formData.value.orderName).then(res => {
			isOrder.value = false
			audioSuccessPlay()
			getLocFocus()
			uni.showToast({
	...
	const submit = () => {
		...
		checkWarehousingcarrier({
			...
			CarrierName: formData.value.barCode
		}).then(res => {
			formData.value.ERPOrder=res.content[0].ERPOrder
			let isEixt = list.value.findIndex(l => l.CarrierName == formData.value.barCode)
			if (isEixt == -1) {
				list.value.unshift({
					CarrierName:formData.value.barCode,
					ProductName:res.content[0].ProductName,
					TotalQty:res.content[0].TotalQty
				})
	...
	const onSubmit = () => {
		list.value.forEach((t)=>{
			formData.value.CarrierName.push(t.CarrierName)
		})
	
		PackingWarehousing(formData.value).then(res => {
			audioSuccessPlay()
			uni.showToast({
				title: '入库成功',
				icon: 'success',
	...
	const swipeClick = (e, data) => {
		list.value = list.value.filter(l => l.CarrierName != data.CarrierName)
	}
